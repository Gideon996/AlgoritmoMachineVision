function [nr,nc,tmpr, tmpc] = trova_cerchi3(I, centri)
...
if(n-tmpr(length(tmpr(:)))>(distanza+10))
    t=n-tmpr(length(tmpr(:)));
    t=t/(distanza);
    t=floor(t)-1;
    for k=p:(p+t)
...
%     p=p+t+1;
% end
if(m-tmpc(length(tmpc(:)))>(distanza+10))
    t=m-tmpc(length(tmpc(:)));
    t=t/(distanza);
	t=floor(t)-1;
    for k=p:(p+t)
